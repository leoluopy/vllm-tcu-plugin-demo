...
from vllm_tcu.worker.model_runner import (
    ModelInputForTCUBuilder, ModelInputForTCUWithSamplingMetadata)
from vllm_tcu.model.gqa import GroupedQueryAttentionDirect
from vllm_tcu.model.pag_att import paged_attention
...
class TCUAttentionBackend(AttentionBackend):
    ...
    @staticmethod
    def get_impl_cls() -> Type["TCUAttentionBackendImpl"]:
        return TCUAttentionBackendImpl

    @staticmethod
    def get_metadata_cls() -> Type["TCUMetadata"]:
        return TCUMetadata
    ...
    @staticmethod
    def get_builder_cls() -> Type["TCUMetadataBuilder"]:
        return TCUMetadataBuilder

    @classmethod
    def make_metadata_builder(cls, *args, **kwargs) -> "TCUMetadataBuilder":
        return cls.get_builder_cls()(*args, **kwargs)


class TCUMLAAttentionBackend(TCUAttentionBackend):

    @staticmethod
    def get_impl_cls() -> Type["TCUMLAAttentionBackendImpl"]:
        return TCUMLAAttentionBackendImpl
...
@dataclass
class TCUMetadata(AttentionMetadata):
    """Metadata for TCUbackend.
    ...
    _cached_prefill_metadata: Optional["TCUMetadata"] = None
    _cached_decode_metadata: Optional["TCUMetadata"] = None
    ...
    @property
    def prefill_metadata(self) -> Optional["TCUMetadata"]:
        ...
        self._cached_prefill_metadata = TCUMetadata(
    ...
    @property
    def decode_metadata(self) -> Optional["TCUMetadata"]:
        ...
        self._cached_decode_metadata = TCUMetadata(
            num_prefills=0,
    ...
    def advance_step(self,
                     model_input: "ModelInputForTCUWithSamplingMetadata",
        ...
        if CUSTOM_OP_ENABLED:
            #advance a step on TCU for existing inputs for a multi-step runner if custom ops is enabled
...
class TCUMetadataBuilder(CommonMetadataBuilder[TCUMetadata]):

    _attn_mask_builder = None  # noqa

    def __init__(self, input_builder: "ModelInputForTCUBuilder"):
        ...
        if TCUMetadataBuilder._attn_mask_builder is None:
            TCUMetadataBuilder._attn_mask_builder = AttentionMaskBuilder.initialize_from_len(
                128, self.input_builder.runner.model_config.dtype)

    def _add_seq_group(
            self, inter_data: ModelInputForTCUBuilder.InterDataForSeqGroup,
            chunked_prefill_enabled: bool):
    ...
    def build(
        ...
        if self.num_prefills > 0:
            if block_tables is None or block_tables.numel() == 0:
                # normal mask
                self.attn_mask = TCUMetadataBuilder._attn_mask_builder.get_attn_mask(  # type: ignore
                ...
                self.compress_mask = TCUMetadataBuilder._attn_mask_builder.get_attn_mask(  # type: ignore
                    128, dtype, device)
            else:
                # chunk_mask for chunk prefill
                attn_mask = TCUMetadataBuilder._attn_mask_builder.get_attn_mask(  # type: ignore
                    max_seq_len, dtype, device)
...
class TCUAttentionBackendImpl(AttentionImpl):
    ...
    def __init__(
        ...
        self.gqa = GroupedQueryAttentionDirect(self.num_heads, 8, 128)
    ...
    def forward(
            ...
            attn_metadata: TCUMetadata,
            attn_type: str = AttentionType.DECODER,
            output: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        """Forward pass with TCU attention.
        Args:
        ...
        if hasattr(layer, 'quant_method'):
            ...
            if attn_metadata.num_prefills > 0:
                ...
                if (attn_metadata.block_tables is None
                    ...
                    if attn_type == AttentionType.ENCODER_ONLY:
                        ...
                        self.seq_lens_tensor_cpu = torch.from_numpy(
                            ...
                            astype(np.int32))
                        # torch.ops._C.flash_attention_qlens(
                        #     query=query,
                        #     key=key,
                        #     value=value,
                        #     mask=mask,
                        ...
                        #     out=output)
                        print("############################################## attention start to invoke ###########")
                ...
                # Prefix cache only and cache hit
                    ...
                    torch_tcu._npu_flash_attention_qlens(
                        query=query,
                        key_cache=self.key_cache,
                        value_cache=self.value_cache,
                ...
                # Splitfuse
                    ...
                    torch_tcu._npu_paged_attention_splitfuse(
                        query=query,
                        key_cache=self.key_cache,
                        value_cache=self.value_cache,
                        block_table=block_tables,
            ...
            # Decode only
                ...
                # torch_tcu._tcu_paged_attention(
                #     query=query,
                #     key_cache=self.key_cache,
                #     value_cache=self.value_cache,
                #     num_kv_heads=self.num_kv_heads,
...
class TCUMLAAttentionBackendImpl(MLAAttentionImpl):
    ...
    def forward(
        ...
        if self.enable_graph_mode:
            if len(kv_cache) > 0 and kv_cache[0].numel(
                ...
                torch_tcu._tcu_reshape_and_cache(key=kv_c_normed.view(
        ...
        if attn_metadata.num_prefills > 0:
            ...
            if (attn_metadata.block_tables is None
                ...
                torch_tcu._tcu_flash_attention(
                    query=query,
                    key=key,
                    value=value,
                    mask=mask,
            ...
            if self.enable_graph_mode:
                ...
                self.seq_lens_tensor_cpu = torch.from_numpy(
                    np.array(attn_metadata.decode_metadata.seq_lens).astype(
                        np.int32))
                block_tables = attn_metadata.decode_metadata.block_tables
                torch_tcu._tcu_paged_attention_mla(
                    query=query,
                    key_cache=kv_cache,
                    num_kv_heads=self.num_kv_heads,
                    num_heads=self.num_heads,
