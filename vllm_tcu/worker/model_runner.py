...
TModelInputForTCU = TypeVar('TModelInputForTCU', bound="ModelInputForTCU")
ENCODER_NUM = 0
@dataclass(frozen=True)
class ModelInputForTCU(ModelRunnerInputBase):
    """
...
@dataclass(frozen=True)
class ModelInputForTCUWithSamplingMetadata(ModelInputForTCU):
    """
    ...
    def from_broadcasted_tensor_dict(
            ...
            tensor_dict: Dict[str, Any],
            attn_backend: Optional["AttentionBackend"] = None,
    ) -> "ModelInputForTCUWithSamplingMetadata":
...
class ModelInputForTCUBuilder(ModelRunnerInputBuilderBase[ModelInputForTCU]):
    """Build ModelInputForTCU from SequenceGroupMetadata."""
    ...
    def prepare(self,
        ...
        # Intermediate data (data in CPU before going to TCU) for
        # the current sequence group.
        self.inter_data_list: List[
            ModelInputForTCUBuilder.InterDataForSeqGroup] = []

        self.attn_metadata_builder.prepare()

    def gen_inter_data_builder(self, num_seqs: int):
        return lambda: ModelInputForTCUBuilder.InterDataForSeqGroup(
            request_id="",
            seq_ids=[0] * num_seqs,
            is_prompt=True,
            block_tables=None,
    ...
    def build(self) -> ModelInputForTCU:
...
class TCUModelRunnerBase(ModelRunnerBase[TModelInputForTCU]):
    """
    Helper class for shared methods between TCU model runners.
    """
    _model_input_cls: Type[TModelInputForTCU]
    _builder_cls: Type[ModelInputForTCUBuilder]
    ...
    def __init__(
        ...
        self.pin_memory = False  # set False first
        print("     @@@ using pin mem: {}".format(self.pin_memory))
    ...
    def remove_all_prompt_adapters(self):
        raise RuntimeError("PromptAdapter is not supported on TCU now.")
...
class TCUModelRunner(TCUModelRunnerBase[ModelInputForTCUWithSamplingMetadata]):
    """
    TCU model runner with sampling step.
    """
    _model_input_cls: Type[ModelInputForTCUWithSamplingMetadata] = (
        ModelInputForTCUWithSamplingMetadata)
    _builder_cls: Type[ModelInputForTCUBuilder] = ModelInputForTCUBuilder
    ...
    def make_model_input_from_broadcasted_tensor_dict(
        ...
        model_input = \
            ModelInputForTCUWithSamplingMetadata.from_broadcasted_tensor_dict(
                tensor_dict,
