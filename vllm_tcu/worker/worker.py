...
from vllm_tcu.worker.model_runner import TCUModelRunner
from vllm_tcu.platform import TCUPlatform

class TCUWorker(LocalOrDistributedWorkerBase):
    """A worker class that executes (a partition of) the model on a TCU.
    Each worker is associated with a single TCU. The worker is responsible for
    maintaining the KV cache and executing the model on the TCU. In case of
    ...
    def __init__(self,
        ...
        self.init_npu_memory =  virtual_mem.total
        print("self.init_npu_memory: {}".format(self.init_npu_memory))
        ...
        ModelRunnerClass: Type[ModelRunnerBase] = TCUModelRunner
        if model_config.runner_type == "pooling":
            ModelRunnerClass = TCUPoolingModelRunner
        ...
        if envs.VLLM_TORCH_PROFILER_DIR:
            ...
            self.profiler = torch_npu.profiler.profile(
                activities=[
                    ...
                    torch_npu.profiler.ProfilerActivity.TCU,
                ],
                with_stack=True,
                profile_memory=True,
                with_modules=True,
    ...
    def sleep(self, level: int = 1) -> None:
        TCUPlatform.set_device(self.device)
        free_bytes_before_sleep = TCUPlatform.mem_get_info()[0]
        allocator = CaMemAllocator.get_instance()
        allocator.sleep(offload_tags=("weights", ) if level == 1 else tuple())
        free_bytes_after_sleep, total = TCUPlatform.mem_get_info()
    ...
    def init_device(self) -> None:
        if self.device_config.device.type == "tcu":
            self.device = torch.device(f"tcu:{self.local_rank}")
            TCUPlatform.set_device(self.device)
            TCUPlatform.empty_cache()
        else:
            raise RuntimeError(
    ...
    @TCUPlatform.inference_mode()
    def determine_num_available_blocks(self) -> Tuple[int, int]:
        """Profiles the peak memory usage of the model to determine how many
        KV blocks may be allocated without OOMs.
        The engine will first conduct a profiling of the existing memory usage.
        Then, it calculate the maximum possible number of TCU and CPU blocks
        that can be allocated with the remaining free memory.
        .. tip::
            You may limit the usage of TCU memory
        ...
        # free_npu_memory, total_npu_memory = TCUPlatform.mem_get_info()
        virtual_mem = psutil.virtual_memory()
        # 总内存（单位：字节）
        total_memory = virtual_mem.total
        # 空闲内存（可用内存，单位：字节）
        free_memory = virtual_mem.available
        free_npu_memory, total_npu_memory = free_memory / 3,total_memory  # TODO use cpu mem first
        print("free_npu_memory:{}, total_npu_memory:{} ".format(free_npu_memory, total_npu_memory ))
        ...
        assert peak_memory > 0, (
            ...
            f" {free_npu_memory}. This happens when the TCU memory was "
            "not properly cleaned up before initializing the vLLM instance.")

        cache_block_size = self.get_cache_block_size_bytes()
        num_npu_blocks = int((
                                     (total_npu_memory * self.cache_config.gpu_memory_utilization -
                                      peak_memory) // cache_block_size) / 2)
        ...
        TCUPlatform.empty_cache()
        print(f"num_npu_blocks:{num_npu_blocks}, num_cpu_blocks:{num_cpu_blocks}")
        return num_npu_blocks, num_cpu_blocks

    def initialize_cache(self, num_gpu_blocks: int,
                         num_cpu_blocks: int) -> None:
        """Allocate TCU and CPU KV cache with the specified number of blocks.
        """
    ...
    def add_prompt_adapter(
        ...
        raise NotImplementedError(
            "Prompt Adapter is not implemented for TCU backend currently.")
    ...
    def _init_worker_distributed_environment(
        ...
        ## for distributed initialize function  put it here
